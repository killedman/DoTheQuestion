...
class Solution:
    def maxSubArray(self, nums: List[int]) -> int:
        # 按照排列组合的方法，遍历所有组合，计算每个组合的大小，选出拥有最大值的组合
        nums_lists = [nums]
        max_sum = sum(nums_lists[0])
        for i in range(len(nums)):
            step = 1
            temp_max_sum = sum([nums[i]])
            while (i+step) < len(nums):
                if sum(nums[i: i + 1 + step]) > temp_max_sum:
                    temp_max_sum = sum(nums[i: i + 1 + step])
                step += 1
